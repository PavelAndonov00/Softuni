function startApp() {
    ...
    const app = Sammy('.content', function () {
        ...
        this.get('#/editPost/:postId', function (ctx) {
            ...
            // let currentPost = post.getPost(ctx.params.postId);
            // currentPost.then(function (res) {
            //     if(res.author !== sessionStorage.getItem('username')) {
            //         alert('not Author')
            //         auth.showError(`You are not author of this post. You cannot change it.`)
            //     } else {
            //         alert('Author')
            //         editEvent(ctx);
            //     }
        ...
        this.get('#/postComments/:postId', function (ctx) {
            commentEvent(ctx);
        });
        ...
        this.post('#/addComment', function (ctx) {
            let author = sessionStorage.getItem('username');
    ...
    function editEvent(ctx) {
        let postPromise = post.getPost(ctx.params.postId);
    ...
    function deleteEvent(ctx) {
        let deletePromise = post.deletePost(ctx.params.postId);
        deletePromise.then(function () {
            ...
            postPromise.then(function (currentPost) {
                ctx.loadPartials({
                    ...
                    this.partial('templates/comments/comments.handlebars', {
                        ...
                        $('#commentForm textarea').on('change', function () {
    ...
    function attachWelcomeEvents() {
        ...
        loginForm.find('input[name=username]').on('change', function () {
            window.location = '#/login'
        });
        loginForm.find('input[name=password]').on('change', function () {
            window.location = '#/login'
        });

        let registerForm = $('#registerForm');
        registerForm.find('input[name=username]').on('change', function () {
            window.location = '#/register'
        });
        registerForm.find('input[name=password]').on('change', function () {
            window.location = '#/register'
        });
        registerForm.find('input[name=repeatPass]').on('change', function () {
            window.location = '#/register'
        });
    }
}